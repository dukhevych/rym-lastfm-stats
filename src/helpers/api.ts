...
interface FetchUserRecentTracksOptions {
  limit?: number;
}

interface LastFmRecentTrackImage {
  size: string;
  '#text': string;
}

interface LastFmRecentTrackArtist {
  mbid: string;
  '#text': string;
}

interface LastFmRecentTrackAlbum {
  mbid: string;
  '#text': string;
}

interface LastFmRecentTrackAttr {
  nowplaying?: string;
}
...
export function fetchUserRecentTracks(
  username: string,
  apiKey: string,
  options: FetchUserRecentTracksOptions = {},
  signal?: AbortSignal
): Promise<LastFmRecentTrack[] | LastFmRecentTracksResponse> {
  ...
  return fetch(url, { signal })
    .then((response) => response.json() as Promise<LastFmRecentTracksResponse>)
    .then((data) => {
