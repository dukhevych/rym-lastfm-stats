...
(async function () {
  window.addEventListener('load', async () => {
    ...
    const parsedData: IRYMRecordDB[] = recentItems.map(item => {
      ...
      const ownershipEl: HTMLElement | null = item.querySelector('.or_q_ownership');

      const rating = ratingEl && ratingEl.src ? parseInt(ratingEl.src.split('/').pop()?.split('.')[0] || '0') : '';
