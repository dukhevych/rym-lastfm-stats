...
async function initSearchResults() {
  ...
  for (const artist of state.artists) {
    const searchAlbumsResponse = await api.search({
      apiKey,
      params: {
        query: normalizeForSearch(artist) + ' ' + titleNormalized,
      },
      searchType: RYMReleaseType.Album,
    });
  ...
  const isFullMatch = (item: SearchResult) => {
    const itemTitleNormalized = normalizeForSearch(item.name);
    const itemArtistNormalized = normalizeForSearch(item.artist);
...
async function render(_config: ProfileOptions) {
  ...
  function prepareMountPoint() {
    ...
    const infoTable = document.querySelector(INFO_TABLE_SELECTOR) as HTMLTableElement;
  ...
  const mountPoint = prepareMountPoint();
